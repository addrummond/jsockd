...
for platform in $platforms; do
    ...
    case "$platform" in
        ...
        windows_x64_msvc)
            ...
            /c/Program\ Files\ */GnuWin32/bin/make.exe CC=../../win-build-utils/fakecl.sh HOST_CC=../../win-build-utils/fakecl.sh AR=../../win-build-utils/fakear.sh CFLAGS="/FI $WPWD\\win-build-utils\\builtins.h /O2 /std:c17 /experimental:c11atomics -Ipthreads-win32-include -D_WINSOCKAPI_ -DWIN32_LEAN_AND_MEAN"
            echo "*** BUILD COMPLETE ***"
            ;;
        mac_arm64)
