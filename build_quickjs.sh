...
if [ -z $JSOCKD_IN_CI ]; then
    ...
    LDFLAGS="-static" CFLAGS="$DEBUG_CFLAGS" make CC=~/filc-0.668.8-linux-x86_64/build/bin/clang CONFIG_LTO= CONFIG_CLANG=y
    ...
    make clean
    git apply ../../fil-c-quickjs.patch
    LDFLAGS="-static" CFLAGS="$RELEASE_CFLAGS" make CC=~/filc-0.668.8-linux-x86_64/build/bin/clang CONFIG_LTO= CONFIG_CLANG=y
