...
for platform in $platforms; do
    ...
    case "$platform" in
        ...
        windows_x64_msvc)
            ...
            echo "*** RUNNING make ***"
            /c/Program\ Files\ */GnuWin32/bin/make.exe CC=../../win-build-utils/fakecl.sh HOST_CC=../../win-build-utils/fakecl.sh AR=.../../win-build-utils/fakear.sh CFLAGS='/nologo /std:c17 /experimental:c11atomics -Ipthreads-win32-include -D_WINSOCKAPI_ -DWIN32_LEAN_AND_MEAN' LDFLAGS='/LIBPATH:pthreads-win32-lib pthreadVC2.lib'
            echo "*** BUILD COMPLETE ***"
            ;;
        mac_arm64)
