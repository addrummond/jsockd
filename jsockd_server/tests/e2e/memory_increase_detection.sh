...
perl -e 'use Time::HiRes qw(usleep); while (<>) { print; usleep(2500); }; sleep(1); print "Sending TERM to $ARGV[0]"; kill "TERM", $ARGV[0]' "$server_pid" < /tmp/jsockd_memory_increase_test_input | ( nc -U /tmp/jsockd_memory_increase_test_sock >/dev/null || true )
