...
#define TYPEOF_BUILTIN_EXPR(x)                                                 \
  _Generic((x),                                                                \
      ...
      default: (void)0)
/* We cannot yield a raw type token; we yield an expression whose type is the
   desired type. So define TYPEOF(x) to be 'typeof_builtin_fallback_t *' pattern
   if you need only sizeof/type traits. Most places expecting a *type* (like
   variable declaration) cannot use this fallback. Provide a helper macro for
   sizeof. */
#define TYPEOF_EXPR(x) TYPEOF_BUILTIN_EXPR(x)
/* Intentionally DO NOT define TYPEOF(x) to avoid misleading usage in
 * declarations. */
#endif
#else
#error                                                                         \
    "No typeof/decltype/_Generic available for MSVC C; cannot provide TYPEOF."
#endif
...
/* TYPEOF_UNQUAL(x):
   Produces the unqualified (non-volatile) version of the type for
   GCC/Clang/etc. Trick: unary + applies usual arithmetic conversions and strips
   top-level qualifiers (and array -> pointer decay). Use carefully. */
#if defined(TYPEOF)
...
#if defined(TYPEOF)
#define TYPEOF_SAME_QUAL(src, expr)                                            \
  TYPEOF(src) /* rely on typeof including qualifiers */ expr
#endif
