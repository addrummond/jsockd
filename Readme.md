...
When the server is ready to start accepting commands on the specified UNIX domain sockets, it prints `READY <integer>` to the standard output followed by a line feed. The integer specifies the number of threads that the server is using to process commands. This may be less than the number of sockets specified, in which case only the specified number of sockets will be used for command processing.
